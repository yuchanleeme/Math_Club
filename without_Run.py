...
class MyApp(QWidget):
    ...
    def initUI(self):
        ...
        self.resize(1200, 800)            # 창의 크기를 정해준다
    ...
    def uploadImg(self):
        ...
        self.layout.addWidget(self.Upload_img, alignment=Qt.AlignCenter)

    def symbolView(self):
        self.next = Symbol_Window()    # 새로운 위젯을 띄우기 위해 Second 클래스 연결

    def historyView(self):
        self.next = History_Window()
...
class History_Window(QWidget):  # History 버튼을 누르면 새로운 위젯이 열리고 리스트에 저장된 수식과 결과를 보여준다
    ...
    def initWindow(self):
        history_list = ['1 + 1 = 2', '2 + 2 = 4', '4 + 4 = 8', '','','','','','','','','','','','','','','']
